class Zoom {
    constructor(settings) {
        ...
        //CUSTOM
        var cursorpos;
        this.imgContainer.addEventListener("mousemove", e => {
        ...
        //document.body.addEventListener("wheel", e => {
            ...
            if(!e.ctrlKey && !e.shiftKey){
                if(e.wheelDelta > 0){
                    let scale = 1 + (e.altKey ? -0.2 : 0.2);
                    this.smoothZomm(this.status.zoom * scale, this.getMousePointer(cursorpos.clientX, cursorpos.clientY));
                } else{
                    let scale = 1 - (e.altKey ? -0.2 : 0.2);
                    this.smoothZomm(this.status.zoom * scale, this.getMousePointer(cursorpos.clientX, cursorpos.clientY));
                }
